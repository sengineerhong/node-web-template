...
//			console.log(tabId);
     ...
     * ****************************************************************************************/
	UtilsIsis.initDaterangepicker = function(id, options)
	{
		$( "#"+id ).daterangepicker($.extend(
			{
				dateLimit: { "months": 1 },
		...
		return $( "#"+id).data('daterangepicker');
	};
...
//			$tId.remove tag 안
	...
	UtilsIsis.setRtSalesHtmlTag = function(key, resParam, $tId)
	{
	    ...
	    // 집계일의 특이사항 여부
		} else if ("Remark" == key.substr(key.length - 6)) {
			if (null == resParam || "" == resParam) resParam = "특이사항 없음";
			$tId.html(resParam);
		// 수량의 경우 포멧터 적용
		} else if ("Amt" == key.substr(key.length - 3) || "Cnt" == key.substr(key.length - 3)) {
			$tId.html(Globalize.formatNumber(resParam));
		// 날짜 포멧변경
		} else if ("Dt" == key.substr(key.length - 2)) {
